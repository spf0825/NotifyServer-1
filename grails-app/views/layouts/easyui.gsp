...
<html>
    ...
    <link rel="stylesheet"  id="themeContent" type="text/css" href="${easyui.path()}/js/easyui/themes/${session.skin?:'default'}/easyui.css">
        ...
        function startSocket1(){
            $.post("${request.contextPath}/message/startSocketServer",null,
                    function (data, textStatus){
                        if(data.result){
                            $.messager.show({
            ...
            $('#sockerButton1').linkbutton('disable');
        }
        function startSocket2(){
            ...
            $('#sockerButton2').linkbutton('disable');
        }
		</g:if>
            function preCloses(){
    ...
    <h1><image src='${easyui.path()}/images/loading/loading11.gif'/><br/>
		...
		<div region="north" title="${message(code:'app.layout.north.title')}" split="true" style="height:100px;padding:10px;">
			<div style="padding:5px;width:auto;text-align:right;float:right;">
                Socket Server 状态1:<g:if test="${SocketServerStatus.isRuning1}">运行1</g:if><g:else>停止2</g:else>
                <a class="easyui-linkbutton" id="sockerButton1" iconCls="icon-ok" <g:if test="${SocketServerStatus.isRuning1}">disabled="true"</g:if> href="javascript:void(0)" onclick="startSocket1()">启动1</a>
                Socket Server 状态2:<g:if test="${SocketServerStatus.isRuning2}">运行2</g:if><g:else>停止2</g:else>
                <a class="easyui-linkbutton" id="sockerButton2" iconCls="icon-ok" <g:if test="${SocketServerStatus.isRuning2}">disabled="true"</g:if> href="javascript:void(0)" onclick="startSocket2()">启动2</a>
