var needInitLayout=true;
    ...
    {field:'bank.id',sortable:true,title:'bank',formatter:bankShowFormatter,width:100,align:'center'},

    {field:'phone.id',sortable:true,title:'phone',formatter:phoneShowFormatter,width:100,align:'center'},
		...
		function insertOk(){
			$('#account-form').form('submit',{
				url:'${request.contextPath}/account/ajaxInsert',
		...
		function editOk(){
			$('#account-form').form('submit',{
				url:'${request.contextPath}/account/ajaxUpdate',
				success:function(data){
					var result=$.trim(data).split(':');
					if(result[0]=='true'){
						$('#account-table').datagrid('reload');
		...
		function editRowData(rowIndex){
			...
			$('#account-form').form('load',allData.rows[rowIndex]);
